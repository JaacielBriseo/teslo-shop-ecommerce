...
const ProductAdminPage: FC<Props> = ({ product }) => {
	...
	const onChangeSize = (size: string) => {
		...
		if (currentSizes.includes(size)) {
			return setValue(
				'sizes',
				currentSizes.filter(size => size !== size),
				{ shouldValidate: true }
			);
		}
				...
				<Grid container spacing={2}>
					...
					<Grid item xs={12} sm={6}>
						<TextField
							...
							{...register('slug', {
								required: 'Este campo es requerido',
								validate: value => (value.trim().includes(' ') ? 'No se puede tener espacios en blanco' : undefined),
							})}
							error={!!errors.slug}
						...
						<Box display='flex' flexDirection='column'>
							...
							<Chip label='Es necesario al menos 2 imagenes' color='error' variant='outlined' sx={{display:getValues('images').length < 2 ? 'flex':'none'}} />
